...
const MainPage = () => {
    ...
    const loadData = (id, type, category) => {
        ...
        axios.get(global.baseURL + "/ideas/api/getIdeas/" + category + "/?" + global.serialize(params)).then(response => {
    ...
    const getCategory = () => {
        setLoading(true);
        axios.get(global.baseURL + "/ideas/api/getCategories/").then(response => {
    ...
    const newVote = (id, index, currentUserIsVote) => {
        currentUserIsVote ?
            axios.post(global.baseURL + "/api/delete/vote/", {idea_id: id}).then(response => {
            ...
            axios.post(global.baseURL + "/ideas/api/newVote/", {idea_id: id, type: "like"}).then(response => {
                if (response.data.state === "success"){
                    let data = [...items];
                    data[index].like += 1;
                    data[index].currentUserIsVote = !currentUserIsVote;
    ...
    const changeStatus = (idea_id, id, name) => {
        axios.post(global.baseURL + "/ideas/api/setStatus/", {idea_id: idea_id, status_id: id}).then(response => {
            if (response.data.state === "success"){
                let data = [...items];
    ...
    const logout = () => {
        axios.post(global.baseURL + "/ru/logout").then(response => {
            if (response.data.state === "success"){
